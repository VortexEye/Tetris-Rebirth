    ...
    def display(self, screen):
        for y in range(self.board_size[0]):
            for x in range(self.board_size[0]):
                if self.grid[y][x] != 0:
                    pygame.draw.rect(screen, self.colors[y][x], (x * self.board_size[2] + 40,
                    60 + y * self.board_size[2], self.board_size[2], self.board_size[2]))

    def add_shape_to_board(self, player):
        for (y, x) in player.get_shape_positions():
